...
import { View, StyleSheet, Text, ActivityIndicator, TouchableOpacity, ScrollView, Alert, Linking } from 'react-native';
...
const DonorTrackingScreen = ({ route, navigation }) => {
  ...
  // Early safety check for hospital coordinates
  useEffect(() => {
    if (!hospitalCoords || !hospitalCoords.latitude || !hospitalCoords.longitude) {
      console.error('DonorTrackingScreen - Invalid hospital coordinates:', hospitalCoords);
      Alert.alert(
        'Location Error',
        'Hospital location coordinates are not available. Returning to previous screen.',
        [{ text: 'OK', onPress: () => navigation.goBack() }]
      );
      return;
    }
  }, [hospitalCoords, navigation]);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      async (snap) => {
        ...
        if (data.donorLocations) {
        ...
        } else if (data && data.donorLocation) {
          ...
          console.log('No donor location data available in request');
        }
        ...
        if (data.donorTrackingEndTime) {
          ...
          // Check if we have any donor location available
          const availableLocation = data && data.donorLocation ? data.donorLocation : Object.values(donorLocations)[0];
          
          if (availableLocation && calculateETA(availableLocation, hospitalCoords)?.eta > 0) {
            // Only set if NOT present in Firestore and not already set locally
            const firstDonorLocation = availableLocation;
            ...
            console.log('No donor location available yet for timer calculation');
          }
        }
        setLoading(false);
      },
  ...
  // Calculate ETA
  const firstDonorLocation = Object.values(donorLocations)[0];
  const etaInfo = firstDonorLocation && hospitalCoords
    ? calculateETA(firstDonorLocation, hospitalCoords)
    : null;
      ...
      {loading ? (
        ...
        <ScrollView style={{width: '100%'}} contentContainerStyle={{alignItems: 'center'}}>
          {donors.map((donor, index) => {
            console.log(`Rendering donor ${index}:`, donor, 'userId:', donor?.userId);
            ...
            // Create a unique key
            const uniqueKey = donor?.userId || donor?.id || `donor-${index}-${donor?.name || 'unknown'}`;
            
            return (
              <View key={uniqueKey} style={styles.infoPanel}>
